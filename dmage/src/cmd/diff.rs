...
pub fn run(ctx: &mut Context, name: &str, show_values: bool) -> Result<(), CliError> {
    ...
    let remote_data = secret::decrypt_secret(
    ...
    let all_keys: BTreeSet<&str> = local_vars.keys().chain(remote_vars.keys()).map(|s| s.as_str()).collect();
...
fn parse_env_map(data: &[u8]) -> std::collections::BTreeMap<String, String> {
    String::from_utf8_lossy(data)
        ...
        .filter_map(|line| {
            ...
            Some((key.trim().to_string(), val.trim_matches('"').trim_matches('\'').to_string()))
        })
        .collect()
}
